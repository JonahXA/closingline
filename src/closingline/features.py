...
_ROLL = [
    ...
    "sotf5", "sota5", "sotf10", "sota10", "conv10", "rest",
]
FEATURE_COLS = ["elo_diff"] + [f"{s}_{c}" for s in _SIDES for c in _ROLL]

# A team record: (date, goals_for, goals_against, points, sot_for, sot_against).
History = dict[str, list[tuple[pd.Timestamp, int, int, int, float, float]]]
...
def build_features(m: pd.DataFrame) -> tuple[pd.DataFrame, History]:
    ...
    hist: History = {}
    rows = []
    for i, (date, h, a, hg, ag, sh, sa) in enumerate(
        zip(m["Date"], m["HomeTeam"], m["AwayTeam"], m["FTHG"], m["FTAG"], hst, ast)
    ):
        rows.append(
            [diffs[i]] + _stats(hist.get(h, []), date) + _stats(hist.get(a, []), date)
        )
        hp = 3 if hg > ag else 1 if hg == ag else 0
        hist.setdefault(h, []).append((date, hg, ag, hp, sh, sa))
        hist.setdefault(a, []).append((date, ag, hg, (3 - hp) if hp != 1 else 1, sa, sh))
